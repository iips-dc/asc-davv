class WinterSchool < ActiveRecord::Base
  ...
  validates :name, :gender, :dob, :religion,:category, :marital_status,:stream, :college,:university,:city,:district,:state,:position,:payscale,:subject,:pin,:university_pin,:date_of_joining,:pref_date1,:pref_date2,:res_address, :email, :image, :course_id, presence: true
  ...
  filterrific(
    ...
    scope :search_query, lambda { |query|
      ...
      where(
        terms.map {
          or_clauses = [
            "LOWER(winter_schools.name) LIKE ?",
            "LOWER(winter_schools.email) LIKE ?",
            "winter_schools.mobile LIKE ?"
          ].join(' OR ')
          "(#{ or_clauses })"
        }.join(' AND '),
    ...
    scope :sorted_by, lambda { |sort_option|
      ...
      when /^created_at_/
        ...
        order("winter_schools.created_at #{ direction }")
      else
